...
def special():
    with open('alpha_beta.json', 'r') as fd:
        map = json.load(fd)
    
    
    srun = sorted(map.iterkeys())
    print "\n".join([str(x) for x in map['20114'][1]])
...
def normal():
    ...
    for i, evt in enumerate(f.event):
        ...
        if not runmap.has_key(evt.rawBurst.nrun):
            ...
            if runmap[evt.rawBurst.nrun][1][-1] != evt.rawBurst.abcog_params.beta:
                runmap[evt.rawBurst.nrun][1].append(evt.rawBurst.abcog_params.beta)
            if runmap[evt.rawBurst.nrun][2][-1] != evt.rawBurst.alpha:
                runmap[evt.rawBurst.nrun][2].append(evt.rawBurst.alpha)
    #     i = i +1
    #     if i>1e4:
    #         break
    
    with open("alpha_beta.json", 'w') as fd:
        json.dump(runmap, fd)
