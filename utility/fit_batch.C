...
void minFct(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t flag){
	...
	for(int i=0; i<=sig->GetNbinsX(); ++i){
		...
		if(totSigma!=0) chi2 += pow((s-fun(G, par[1], b1,b2,b3)),2.)/totSigma;
	}
...
void drawFitResult(const fitResult& result, TString proc){
	...
	for(int i=0; i<inputMCNbr; ++i){
		TH1D *d3_c = (TH1D*)d3->at(i)->Clone(TString::Format("d3_c%s%i", proc.Data(), i));
		leg->AddEntry(d3_c, TString::Format("%s FF=x^{2}", mcLegendTitle[i].Data()));
		d3_c->Scale(gWeight*a*a);
		stack->Add(d3_c);
	}
	for(int i=0; i<inputMCNbr; ++i){
		TH1D *d2_c = (TH1D*)d2->at(i)->Clone(TString::Format("d2_c%s%i", proc.Data(), i));
		leg->AddEntry(d2_c, TString::Format("%s FF=x", mcLegendTitle[i].Data()));
		d2_c->Scale(gWeight*2.*a);
		stack->Add(d2_c);
	}
	for(int i=0; i<inputMCNbr; ++i){
		TH1D *d1_c = (TH1D*)d1->at(i)->Clone(TString::Format("d1_c%s%i", proc.Data(), i));
		leg->AddEntry(d1_c, TString::Format("%s FF=1", mcLegendTitle[i].Data()));
		d1_c->Scale(gWeight);
		stack->Add(d1_c);
	}
...
double procedure1(fitResult& result){
	//Initialize MINUIT
...
double procedure2(fitResult& result){
	//Initialize MINUIT
	...
	minuit.mnparm(0, "G", 1., 100, 0, 0, flag);
	minuit.mnparm(1, "a", 0.05, 0.001, 0, 0, flag);
	minuit.mnparm(2, "fix", 0.0, 0, 0, 0, flag);
	minuit.FixParameter(2);
	minuit.FixParameter(0);
...
double procedureNew(fitResult& result){
	//Initialize MINUIT
...
double procedureROOT(fitResult& result){
	//Initialize MINUIT
...
void chi2Profile(fitResult r, TString name){
	...
	for(int i=0; i<100; i++){
		...
		minFct(npar, NULL, chi2Value, params, 0);
