...
void buildRunMap(TFile *fd, map<int, double> &runMap, bool usePk) {
	...
	for (int i = 0; i < nevt; ++i) {
		...
		if (runMap.count(burstBrch->nrun) > 0)
			runMap[burstBrch->nrun] += 1. * weight;
		else
			runMap.insert(make_pair(burstBrch->nrun, 1. * weight));
	}
}
...
void buildRatioMap() {
	...
	for (it = ratio.begin(); it != ratio.end(); ++it) {
		cout << "Run " << it->first << ": " << it->second << "("
				<< mcRunMap[it->first] << "/" << dataRunMap[it->first] << endl;
		fd << it->first << " " << it->second << " " << mcRunMap[it->first]
				<< " " << dataRunMap[it->first] << endl;
	}
