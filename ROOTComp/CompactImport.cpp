...
CompactImport::CompactImport(): currentEvent(-1), currentFile(-1),
	nEvents(0), nFiles(0), mcBranched(false), currentFileName(),
	inTree(new TChain("event")), headerTree(new TChain("header")),
	rawEvent_ptr(nullptr), corrEvent_ptr(nullptr), rootBurst_ptr(nullptr),
	rootFileHeader_ptr(nullptr), Geom(nullptr), rootMC_ptr(nullptr)
{
}
...
bool CompactImport::eof() {
	if(currentEvent==nEvents) return true;
	else return false;
}
