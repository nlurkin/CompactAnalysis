...
int nico_pi0DalitzSelect(){
	...
	if(rootBurst.isData){
		...
		if(fabs(rawEvent.track[corrEvent.pTrack[corrEvent.goodTracks[0]].trackID].time - rootBurst.tOffst.Dch)>=io.cutsDefinition.maxTrackTime) {pi0d_failCut(6); return -1;}
		...
		if(fabs(rawEvent.track[corrEvent.pTrack[corrEvent.goodTracks[1]].trackID].time - rootBurst.tOffst.Dch)>=io.cutsDefinition.maxTrackTime) {pi0d_failCut(6); return -1;}
		...
		if(fabs(rawEvent.track[corrEvent.pTrack[corrEvent.goodTracks[2]].trackID].time - rootBurst.tOffst.Dch)>=io.cutsDefinition.maxTrackTime) {pi0d_failCut(6); return -1;}
	}
	else{
	...
	if(badAcceptance==io.cutsDefinition.boolBadTrack) {pi0d_failCut(7); return -1;}
	...
	if(piCandNb!=io.cutsDefinition.numPiCandidates) {pi0d_failCut(9); return -1;}
	...
	if(badCombis!=io.cutsDefinition.numBadTrackCombi) {pi0d_failCut(8); return -1;}
	...
	if(badElectron==io.cutsDefinition.boolBadECandidates) {pi0d_failCut(10); return -1;}
	...
	if(goodClusters!=io.cutsDefinition.numAddGoodCluster) {pi0d_failCut(12); return -1;}
	...
	if(lkrAcceptance!=io.cutsDefinition.lkrAcceptance) {pi0d_failCut(13); return -1;}
	...
	if(rootPhysics.gamma.P.E()<=io.cutsDefinition.minGammaEnergy) {pi0d_failCut(14); return -1;}
	...
	if(rawEvent.cluster[corrEvent.pCluster[rootPhysics.gamma.parentCluster].clusterID].dDeadCell<=io.cutsDefinition.minDeadCellDist) {pi0d_failCut(15); return -1;}
	...
	if(rawEvent.track[corrEvent.pTrack[rootPhysics.pic.parentTrack].trackID].dDeadCell<=io.cutsDefinition.minDeadCellDist) {pi0d_failCut(15); return -1;}
	...
	if(rawEvent.track[corrEvent.pTrack[rootPhysics.ep.parentTrack].trackID].dDeadCell<=io.cutsDefinition.minDeadCellDist) {pi0d_failCut(15); return -1;}
	...
	if(rawEvent.track[corrEvent.pTrack[rootPhysics.em.parentTrack].trackID].dDeadCell<=io.cutsDefinition.minDeadCellDist) {pi0d_failCut(15); return -1;}
	...
	if(radius<=io.cutsDefinition.minGammaDCHRadius) {pi0d_failCut(16); return -1;}
	...
	if(corrEvent.pTrack[corrEvent.goodTracks[piTrack]].p<=io.cutsDefinition.minTrackMomentum || corrEvent.pTrack[corrEvent.goodTracks[piTrack]].p>=io.cutsDefinition.maxTrackMomentum) {pi0d_failCut(11); return -1;}
	if(corrEvent.pTrack[corrEvent.goodTracks[epTrack]].p<=io.cutsDefinition.minTrackMomentum || corrEvent.pTrack[corrEvent.goodTracks[epTrack]].p>=io.cutsDefinition.maxTrackMomentum) {pi0d_failCut(11); return -1;}
	if(corrEvent.pTrack[corrEvent.goodTracks[emTrack]].p<=io.cutsDefinition.minTrackMomentum || corrEvent.pTrack[corrEvent.goodTracks[emTrack]].p>=io.cutsDefinition.maxTrackMomentum) {pi0d_failCut(11); return -1;}
	...
	if(rootPhysics.kaon.P.Vect().Mag()<io.cutsDefinition.minTotalMomentum || rootPhysics.kaon.P.Vect().Mag()>io.cutsDefinition.maxTotalMomentum) {pi0d_failCut(17); return -1;}
	...
	if(pt>=io.cutsDefinition.maxPt) {pi0d_failCut(18); return -1;}
	...
	if(fabs(rootPhysics.pi0.P.M()-Mpi0)>=io.cutsDefinition.maxPi0MassDiff) {pi0d_failCut(19); return -1;}
	...
	if(rootPhysics.kaon.P.M()<io.cutsDefinition.minKaonMassDiff || rootPhysics.kaon.P.M()>io.cutsDefinition.maxKaonMassDiff) {pi0d_failCut(20); return -1;}
